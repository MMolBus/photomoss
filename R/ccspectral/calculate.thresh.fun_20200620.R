...
calculate.thresh.fun <- function(list.raster.results, threshold.method){
  ...
  if (any(unique(grepl(index_order[1], index.))) == TRUE) { 
    ...
    t_values[[1]] <-  autothreshold.value(raster = ri,
                                        max.index = 1,
                                        min.index = -1,
                                        method    = threshold.method
                                        )
    raster_index_cut[[1]]  <- raster(1*as.matrix(ri >= t_values[[1]]))
  ...
  if (any(unique(grepl(index_order[2], index.))) == TRUE) {
    ...
    t_values[[2]] <- autothreshold.value(raster    = ri,
                                       max.index = max(getValues(ri), 
                                                       na.rm = T),
                                       min.index = 0,
                                       method    = threshold.method
                                       )
    raster_index_cut[[2]]  <- raster(1*as.matrix(ri >= t_values[[2]]))
    } else{
      t_values[[2]]<- NULL
  ...
  if (any(unique(grepl(index_order[3], index.))) == TRUE) {
    ...
    t_values[[3]]  <- autothreshold.value(raster    = ri,
                                        max.index = 1,
                                        min.index = -1,
                                        method    = threshold.method
                                        )
    raster_index_cut[[3]]  <- raster(1*as.matrix(ri >= t_values[[3]]))
    } else{
      t_values[[3]] <- NULL
  ...
  if (any(unique(grepl(index_order[4], index.))) == TRUE) {
    ...
    t_values[[4]] <- autothreshold.value(raster    = ri,
                                       max.index = 1,
                                       min.index = -1,
                                       method    = threshold.method
                                       )
    raster_index_cut[[4]]  <- raster(1*as.matrix(ri >= t_values[[4]]))
    } else{
      t_values[[4]] <- NULL
  ...
  if (any(unique(grepl(index_order[5], index.))) == TRUE) {
    ...
    t_values[[5]] <- autothreshold.value(raster    = ri,
                                       max.index = 2,
                                       min.index = 0,
                                       method    = threshold.method
                                       )
    
    raster_index_cut[[5]]  <- raster(1*as.matrix(ri <= t_values[[5]]))
    } else{
      t_values[[5]]<- NULL
  ...
  if (any(unique(grepl(index_order[6], index.))) == TRUE) {
    ...
    t_values[[6]] <- autothreshold.value(raster  = ri,
                                       max.index = max(getValues(ri)
                                                       , na.rm = T),
                                       min.index = 0,
                                       method    = threshold.method
                                       )
    raster_index_cut[[6]]  <- raster(1*as.matrix(ri >= t_values[[6]]))
    } else{
      t_values[[6]]<- NULL
  ...
  if (any(unique(grepl(index_order[7], index.))) == TRUE) {
    ...
    t_values[[7]] <- autothreshold.value(raster    = ri,
                                       max.index = 2,
                                       min.index = 0,
                                       method    = threshold.method
                                       )
    raster_index_cut[[7]]  <- raster(1*as.matrix(ri <= t_values[[7]]))
    } else{
      t_values[[7]]<- NULL
  ...
  if (any(unique(grepl(index_order[8], index.))) == TRUE) {
    ...
    t_values[[8]] <- autothreshold.value(raster    = ri,
                                       max.index = 1,
                                       min.index = 0,
                                       method    = threshold.method
                                       )
    
    raster_index_cut[[8]]  <- raster(1*as.matrix(ri >= t_values[[8]]))
    } else{
      t_values[[8]]<- NULL
  ...
  if (any(unique(grepl(index_order[9], index.))) == TRUE) {
    ...
    t_values[[9]] <- autothreshold.value(raster    = ri,
                                       max.index = 1,
                                       min.index = 0,
                                       method    = threshold.method
                                       )
    raster_index_cut[[9]]  <- raster(1*as.matrix(ri <= t_values[[9]]))
    } else{
      t_values[[9]] <- NULL
  ...
  if (any(unique(grepl(index_order[10], index.))) == TRUE) {
    ...
    t_values[[10]] <- autothreshold.value(raster    = ri,
                                        max.index = 1,
                                        min.index = 0,
                                        method    = threshold.method
                                        )
    raster_index_cut[[10]] <- raster(1*as.matrix(ri <= t_values[[10]]))
    } else{
      t_values[[10]]<- NULL
  ...
  if (any(unique(grepl(index_order[11], index.))) == TRUE) {
    ...
    t_values[[11]] <- autothreshold.value(raster    = ri,
                                        max.index = 1.4,
                                        min.index = -1,
                                        method    = threshold.method
                                        )
    raster_index_cut[[11]] <- raster(1*as.matrix(ri >= t_values[[11]]))
    } else{
      t_values[[11]] <- NULL
  ...
  if (any(unique(grepl(index_order[12], index.))) == TRUE) {
    ...
    t_values[[12]] <- autothreshold.value(raster    = ri,
                                        max.index = 2,
                                        min.index = -2,
                                        method    = threshold.method
                                        )
    raster_index_cut[[12]] <- raster(1*as.matrix(ri <= t_values[[12]]))
    } else{
      t_values[[12]]<- NULL
  ...
  if (any(unique(grepl(index_order[13], index.))) == TRUE) {
    ...
    t_values[[13]] <- autothreshold.value(raster    = ri,
                                        max.index = 1.4,
                                        min.index = -1,
                                        method    = threshold.method
                                        )
    raster_index_cut[[13]] <- raster(1*as.matrix(ri <= t_values[[13]]))
    } else{
      t_values[[13]]<- NULL
  ...
  if (any(unique(grepl(index_order[14], index.))) == TRUE) {
    ...
    t_values[[14]] <- autothreshold.value(raster    = ri,
                                        max.index = 3,
                                        min.index = -3.4,
                                        method    = threshold.method
                                        )
    raster_index_cut[[14]] <- raster(1*as.matrix(ri <= t_values[[14]]))
    } else{
    t_values[[14]] <- NULL
  ...
  if (any(unique(grepl(index_order[15], index.))) == TRUE) {
    ...
    t_values[[15]] <- autothreshold.value(raster    = ri,
                                        max.index = 19.61345,
                                        min.index = 17.97645,
                                        method    = threshold.method
                                        )
    raster_index_cut[[15]] <- raster(1*as.matrix(ri >= t_values[[15]]))
    } else{
      t_values[[15]] <- NULL
  ...
  if (any(unique(grepl(index_order[16], index.))) == TRUE) {
    ...
    t_values[[16]] <- autothreshold.value(raster    = ri,
                                        max.index = max(values(ri),
                                                        na.rm = T),
                                        min.index = 0,
                                        method    = threshold.method
                                        )
    raster_index_cut[[16]] <- raster(1*as.matrix(ri <= t_values[[16]]))
    } else{
      t_values[[16]] <- NULL
  ...
  if (any(unique(grepl(index_order[17], index.))) == TRUE) {
    ...
    t_values[[17]] <- autothreshold.value(raster    = ri,
                                        max.index = 1,
                                        min.index = 0,
                                        method    = threshold.method
                                        )
    raster_index_cut[[17]] <- raster(1*as.matrix(ri <= t_values[[17]]))
    } else{
      t_values[[17]] <- NULL
  ...
  if (any(unique(grepl(index_order[18], index.))) == TRUE) {
    ...
    t_values[[18]] <- autothreshold.value(raster    = ri,
                                          max.index = 1,
                                          min.index = 0,
                                          method    = threshold.method
                                        )
    raster_index_cut[[18]] <- raster(1*as.matrix(ri >= t_values[[18]]))
    } else{
      t_values[[18]] <- NULL
  ...
  if (any(unique(grepl(index_order[19], index.))) == TRUE) {
    ...
    t_values[[19]] <- autothreshold.value(raster    = ri,
                                        max.index = 1,
                                        min.index = 0,
                                        method    = threshold.method
                                        )
    raster_index_cut[[19]] <- raster(1*as.matrix(ri <= t_values[[19]]))
    } else{
    t_values[[19]] <- NULL
