...
ccSpectral.multiareas <- function(tif.path, chart, obs.areas, #sample.names=NULL,
  ...
  # NIR
    ...
    rednorm.t    <- autotreshold.value(red.norm,  1, -1, "mean", rref= vis.red)
    bluenorm.t   <- autotreshold.value(blue.norm, 1, -1, "mean", rref= vis.red)
    ...
    # New index
    rednorm.t <- red.norm >= thresholds[2]
    bluenorm.t <- blue.norm <= thresholds[3]
    excessblue.t <- ex.blue <= thresholds[4]
