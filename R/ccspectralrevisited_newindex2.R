# ccspectarl con automatización de tresholds
...
ccSpectral.multiareas <- function(tif.path, chart, obs.areas, #sample.names=NULL,
  ...
  #create the exportation directory for results
  out.dir <- paste("output", Sys.time())
  dir.create(out.dir)
  #results data.frame columns
  # df <- data.frame("unit"=character()      , "vis.file"=character(), "nir.file"=character()     , 
  #                  "red.rsq"=numeric()     , "green.rsq"=numeric() , "blue.rsq"=numeric()       , "nir.rsq"=numeric()    , 
  #                  "ndvi.median"=numeric() , "ndvi.mean"=numeric() , "ndvi.threshold"=numeric() , "ndvi.cover"=numeric() ,
  #                  "sr.median"=numeric()   , "sr.mean"=numeric()   , "sr.threshold"=numeric()   , "sr.cover"=numeric()   , 
  #                  "msavi.median"=numeric(), "msavi.mean"=numeric(), "msavi.threshold"=numeric(), "msavi.cover"=numeric(), 
  #                  "evi.median"=numeric()  , "evi.mean"=numeric()  , "evi.threshold"=numeric()  , "evi.cover"=numeric()  ,
  #                  "ci.median"=numeric()   , "ci.mean"=numeric()   , "ci.threshold"=numeric()   , "ci.cover"=numeric()   ,  
  #                  "bsci.median"=numeric() , "bsci.mean"=numeric() , "bsci.threshold"=numeric() , "bsci.cover"=numeric() ,
  #                  "bi.median"=numeric()   , "bi.mean"=numeric()   , "bi.threshold"=numeric()   , "bi.cover"=numeric()) 
  #                  
  # 
  df <- data.frame("unit"=character()          ,"vis.file"=character()    ,"nir.file"=character()         , 
                   "red.rsq"=numeric()         ,"green.rsq"=numeric()     ,"blue.rsq"=numeric()           ,"nir.rsq"=numeric()        , 
                   "ndvi.median"=numeric()     ,"ndvi.mean"=numeric()     ,"ndvi.threshold"=numeric()     ,"ndvi.cover"=numeric()     , 
                   "red.norm.median"=numeric() ,"red.norm.mean"=numeric() ,"red.norm.threshold"=numeric() ,"red.norm.cover"=numeric() , 
                   "blue.norm.median"=numeric(),"blue.norm.mean"=numeric(),"blue.norm.threshold"=numeric(),"blue.norm.cover"=numeric(),
                   "ex.blue.median"=numeric()  ,"ex.blue.mean"=numeric()  ,"ex.blue.threshold"=numeric()  ,"ex.blue.cover"=numeric()  ) 
                   # "ci.median"=numeric()       ,"ci.mean"=numeric()       ,"ci.threshold"=numeric()       ,"ci.cover"=numeric()       ,
                   # "bsci.median"=numeric()     ,"bsci.mean"=numeric()     ,"bsci.threshold"=numeric()     ,"bsci.cover"=numeric()     ,
                   # "bi.median"=numeric()       ,"bi.mean"=numeric()       ,"bi.threshold"=numeric()       ,"bi.cover"=numeric())
  ...
  #total samples per picture (same number of samples in each picture )
  ...
  # NIR
    ...
    raster.mat <- brick(red.mat, green.mat, blue.mat)
    ...
    ndvi <- (nir.mat - red.mat)/(nir.mat + red.mat)
     ...
     rgb.spectral.normalization <- function(r) {
       ...
       red.norm <-
         raster::setValues (rgb.norm[[1]], (
           raster::getValues(r[[1]]) / (
             raster::getValues(r[[1]]) + raster::getValues(r[[2]]) + raster::getValues(r[[3]])
           )
         ))
       
       
       green.norm <-
         raster::setValues(rgb.norm[[2]], (
           raster::getValues(r[[2]]) / (
             raster::getValues(r[[1]]) + raster::getValues(r[[2]]) + raster::getValues(r[[3]])
           )
         ))
       
       blue.norm <-
         raster::setValues(rgb.norm[[3]], (
           raster::getValues(r[[3]]) / (
             raster::getValues(r[[1]]) + raster::getValues(r[[2]]) + raster::getValues(r[[3]])
           )
         ))
    ...
    rgb.norm <- rgb.spectral.normalization(raster.mat)
    
    # normalized red
    # red.norm  <- getValues(rgb.norm[[1]])  
    red.norm  <- rgb.norm[[1]]  
    # normalized blue
    # blue.norm <- getValues(rgb.norm[[3]])  
    blue.norm <- rgb.norm[[3]]  
    # Excess blue
    ex.blue <- raster(rgb.norm[[3]])
    v <- 1.4 * getValues(rgb.norm[[3]]) - getValues(rgb.norm[[2]])
    ex.blue <- setValues(ex.blue, v)
    
    # # normalized red
    # red.norm  <- raster::getvalues(rgb.norm[[1]])  
    # # normalized blue
    # blue.norm <- raster::getvalues(rgb.norm[[3]])  
    # # Excess blue
    ...
    #####################################################33
    # rgb.spectral.normalization <- function(r) {
    #   red.normalized.coordinates <- raster::getValues(r[[1]]) / max(raster::getValues(r[[1]]))
    #   green.normalized.coordinates <- raster::getValues(r[[2]]) / max(raster::getValues(r[[2]]))
    #   blue.normalized.coordinates <- raster::getValues(r[[3]]) / max(raster::getValues(r[[3]]))
    #   rgb.norm <- raster::raster(r)
    #   red.norm <-  raster::setValues (rgb.norm, 
    #                                   (red.normalized.coordinates / 
    #                                      ( red.normalized.coordinates + green.normalized.coordinates + blue.normalized.coordinates)
    #                                    )
    #                                   )
    #   green.norm <- raster::setValues(rgb.norm, 
    #                                   (green.normalized.coordinates / 
    #                                      (red.normalized.coordinates + green.normalized.coordinates + blue.normalized.coordinates)
    #                                    )
    #                                   )
    #   blue.norm <- raster::setValues(rgb.norm, 
    ...
    #####################################################
    ####################INTRODUCING AUTOTHRESHOLDS#################################
    autotreshold.value <- function(r, max.index, min.index, method, rref) {
      col.0.255.bin.range <- function (r, max.index, min.index) {
        a <- max.index - min.index
        bin <-
          .bincode(as.vector(r),
                   seq(min.index, max.index, (a / 256)),
                   include.lowest = T)
        return(bin)
      }
      col.code.index <- col.0.255.bin.range(r, max.index, min.index)
      index.values <- values(r)
      df.colcode.indexvalues <-
      ...
      p <-
        sort(unique(df.colcode.indexvalues$colcode), decreasing = F)[pt]
      out <-
        min((df.colcode.indexvalues[df.colcode.indexvalues$colcode == p, ])$indexvalues)
      
      print(out)
      
      # # plot== logical()
      #   # if(plot=="TRUE"){
      #     index <- df.colcode.indexvalues$indexvalues < out
      #     # df.colcode.indexvalues$indexvalues
      #     # df.colcode.indexvalues$colcode[index] <- (df.colcode.indexvalues$colcodes[index] - 5)/2.533 
      #     df.colcode.indexvalues$colcode[index] <- 0
      #     values(r) <- df.colcode.indexvalues$colcode
      #     df.colcode.indexvalues$indexvalues[index] <- 0
      #     values(r)<-  df.colcode.indexvalues$indexvalues
      ...
      #                                     alpha = NULL) 
      #     plot(r, col=grayscale_colors, asp = nrow(rref)/ncol(rref)
      #          #,main=paste0( deparse(substitute(ndvi), method, "tresh =", out))
      #          )
      #   # }else{NULL}
    }
    
    # ndvi.t <- autotreshold.value(ndvi, 1, -1, "mean", rref= vis.red)
    # 
    # sr.t <- autotreshold.value(sr, max(values(sr), na.rm = T), 0, "mean", rref= vis.red)
    # 
    # msavi.t <- autotreshold.value(msavi, 1, -1, "mean", rref= vis.red)
    # 
    # evi.t <- autotreshold.value(evi, max(values(evi), na.rm = T), min(values(evi), na.rm = T), "mean", rref= vis.red)
    # 
    # # evi.t <- autotreshold.value(evi, 1.5, -1.5, "mean", rref= vis.red)
    # 
    ...
    rednorm.t    <- autotreshold.value(red.norm,  1, 0, "mean", rref= vis.red)
    bluenorm.t   <- autotreshold.value(blue.norm, 1, 0, "mean", rref= vis.red)
    excessblue.t <- autotreshold.value(ex.blue,   1, -1, "mean", rref= vis.red)
    ...
    # # MODIFIED CODE
    # ndvi.cut <- ndvi >= ndvi.t
    # sr.cut <- sr >= sr.t
    # msavi.cut <- msavi >= msavi.t
    # evi.cut <- evi >= evi.t
    # ci.cut <- ci >= ci.t
    # bsci.cut <- bsci >= bsci.t
    # bi.cut <- bi <= bi.t
   
     # Original index autothreshold vector
    # thresholds <- c(ndvi.t, sr.t, msavi.t, evi.t, ci.t, bsci.t, bi.t )
    
    # new index autothreshold vector 
    thresholds <- c(ndvi.t, rednorm.t, bluenorm.t, excessblue.t )
    
    
    #ORIGINAL CODE
    ndvi.cut <- ndvi >= thresholds[1]
    # New index
    red.norm.cut <- red.norm <= thresholds[2]
    blue.norm.cut <- blue.norm >= thresholds[3]
    ex.blue.cut <- ex.blue >= thresholds[4]
    ...
    if(pdf == T){
      ...
      hist(red.norm, breaks = 1000, main = "red.norm Distribution")
      plot(red.norm, col = pal, main = "red.norm Values", axes = FALSE,
           box = FALSE, asp = asp)
      plot(red.norm.cut, col = c("black", "green"), legend = F, main = paste("red.norm Binary Cover threshold",
                                                                       thresholds[2]), axes = FALSE, box = FALSE, asp = asp)
      hist(blue.norm, breaks = 1000, main = "blue.norm Distribution")
      plot(blue.norm, col = pal, main = "blue.norm Values", axes = FALSE,
           box = FALSE, asp = asp)
      plot(blue.norm.cut, col = c("black", "green"), legend = F,
           main = paste("blue.norm Binary Cover threshold", thresholds[3]),
           axes = FALSE, box = FALSE, asp = asp)
      hist(ex.blue, breaks = 1000, main = "ex.blue Distribution")
      plot(ex.blue, col = pal, main = "ex.blue Values", axes = FALSE,
           box = FALSE, asp = asp)
      plot(ex.blue.cut, col = c("black", "green"), legend = F,
           main = paste("ex.blue Binary Cover threshold", thresholds[4]),
           axes = FALSE, box = FALSE, asp = asp)
    ...
    #NEW index
    red.norm.mean <- cellStats(red.norm, stat = "mean")
    red.norm.median <- median(na.omit(values(red.norm)))
    red.norm.cover <- nrow(rasterToPoints(reclassify(red.norm.cut, rcl = cbind(0, 
                                                                   NA))))/nrow(obs.area)
    blue.norm.mean <- cellStats(blue.norm, stat = "mean")
    blue.norm.median <- median(na.omit(values(blue.norm)))
    blue.norm.cover <- nrow(rasterToPoints(reclassify(blue.norm.cut, 
                                                  rcl = cbind(0, NA))))/nrow(obs.area)
    ex.blue.mean <- cellStats(ex.blue, stat = "mean")
    ex.blue.median <- median(na.omit(values(ex.blue)))
    ex.blue.cover <- nrow(rasterToPoints(reclassify(ex.blue.cut, 
                                                rcl = cbind(0, NA))))/nrow(obs.area)
    ...
    new.dat <- data.frame(sample.name, vis.photo, nir.photo, 
                          ...
                          ndvi.mean, thresholds[1], ndvi.cover, red.norm.median, 
                          red.norm.mean, thresholds[2], red.norm.cover, blue.norm.mean, blue.norm.median, 
                          thresholds[3], blue.norm.cover, ex.blue.mean, ex.blue.median, 
                          thresholds[4], ex.blue.cover)  
    #OLD index
    # new.dat <- data.frame(sample.name, vis.photo, nir.photo, 
    #                       red.rsq, green.rsq, blue.rsq, nir.rsq, ndvi.median, 
